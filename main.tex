...
\begin{figure}[htbp!]
	...
	\caption{Measured $\sigma_{pd}/2\sigma_{pp}$ Drell-Yan cross section ratio from the two SeaQuest datasets compared calculations using
		...
		% (top). As well as comparisons with E866/NuSea~\cite{towell2001} and calculations using CT18~\cite{hou2021} at different kinematics (bottom).}
	\label{fig:xT_csr}
	\todo{which PDFs to use? Also, check error on PDF, are those 68\% CL?}
\end{figure}
...
\begin{table*}[htbp!]
	...
	\end{adjustbox}
	\todo{check error on PDF, are those 68\% CL?}
\end{table*}
...
\begin{figure}[htpb!]
	...
	\todo{need better color scheme! Also, check error on PDF, are those 68\% CL?}
\end{figure}
