...
section ratios obtained from the analysis of the two SeaQuest
data sets (Part A and Part B) are shown in \Cref{fig:xT_csr}. 
...
\Cref{fig:xT_csr} shows that the results from the two separate data sets 
are largely consistent 
with each other, except for the lowest $x_2$ bin where the SeaQuest
spectrometer acceptance is small and more sensitive to the trigger 
conditions. The results from the two data sets on $\sigma_{pd}/2\sigma_{pp}$
are combined, as shown in \Cref{fig:xT_csr} and tabulated 
in \cref{tab:dbarubar}. \Cref{fig:xT_csr} compares the SeaQuest results with
NLO calculations using DYNNLO~\cite{catani2007,catani2009},
with nucleon PDFs CT18~\cite{hou2021} and NNPDF4.0~\cite{ball2022a}. Note that
CT18~\cite{hou2021} was obtained without input from the first 
SeaQuest result~\cite{dove2021}, while the first SeaQuest result was included 
in the global fit for NNPDF4.0. This accounts for the better agreement between
the data and the NNPDF4.0 calculation, although the data tend to be higher than
the calculation for the large $x$ region. Nevertheless, \Cref{fig:xT_csr}
...
\begin{figure*}[htpb!]
	...
	\todo[author=JCPeng,color=red]{use black for 
	SeaQuest data and red for E866}
\end{figure*}
...
from CT18~\cite{hou2021} and NNPDF4.0~\cite{ball2022a}
...
\begin{acknowledgments}
	...
	\todo{Add funding information. For FNAL, should it be listed as FRA or FermiForward?}
\end{acknowledgments}
